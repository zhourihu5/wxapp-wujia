...
Page({
  ...
  loadData(id) {
    let that = this
    network.requestGet('/v1/activity/isOrder', {
      activityId: id
    }, function(data) {
      that.setData({
        apiData: data,
  ...
  toPay(e) {
    ...
    var paramData = {
      ...
      giftId: that.data.apiData.giftId
    }
    if (this.data.couponAct) {
    ...
    console.log("下单参数 paramData --- ", paramData, that)
  ...
  payOrder() { //修改订单状态
    var that = this
    network.requestPost('/v1/order/payOrder', {
      id: that.data.apiPayOrderData.id
    }, function(data) {
      app.activityChanged = true
      wx.redirectTo({
        url: "/pages/paySuccess/index?id=" + that.data.apiPayOrderData.id
      })
    }, function(msg) {
